...
// 订阅
func (c *Client) Subscribe(channel string) (bool , map[string]map[int64]*Client) {
	c.mu.Lock()
	defer c.mu.Unlock()
	//判断channel这个键是否存在
	//存在直接返回false
	...
	//不存在channel这个键，则调sh里的Subscribe函数进行真正的订阅操作
	test := NewSubscriptionHub()
	var ok bool

	if ok, test.Subscribers = c.sh.Subscribe(channel, c); ok {
		c.channels[channel] = struct{}{}
		return true, test.Subscribers
	}
